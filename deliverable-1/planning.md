...
When any vote needs to be done, we have various methods of polling the group members. For voting on meeting times, we use a server called when2meet which is designed to help groups determine who is busy and when to find the optimal meeting times. If there are any ties, we use a regular vote to tiebreak the most popular choices; this is the same two stage voting process we used to determine our weekly meeting time. For regular votes, we use Google forms as it is the most powerful option, and has useful features such as allowing anonimity and filtering results.
